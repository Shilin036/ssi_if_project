...
class ShowerDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        voxel_info = torch.tensor(self._file_handle['voxels'][idx].reshape(-1, 6), dtype=torch.float32)
        pos_x = voxel_info[:, 0]
        pos_y = voxel_info[:, 1]
        pos_z = voxel_info[:, 2]
        pos = np.stack((pos_x, pos_y, pos_z), axis = 0)
        gFeatureArray = np.cov(pos).reshape(1, -1)
        #gFeatureArray.append(np.std(voxel_info[:, 0,1,2])) variance in each direction
        gFeatureArray = np.append(gFeatureArray, len(voxel_info))
        gFeatureArray = gFeatureArray.reshape(1, -1)
        global_features = torch.tensor(gFeatureArray, dtype=torch.float32)
        global_label = len(set(voxel_info[:, 4].numpy()))
		#### Extract global features
...
#for data in loader:
#    print(np.max(data.global_label.numpy()))

print(data.global_attr[0:5])
print(data.global_label[0:5])
print(data.x.shape)

import torch.nn as nn # Neural Network implementations in PyTorch

class EdgeLayer(nn.Module):
    def __init__(self, global_in, node_in, edge_in, edge_out, leakiness=0.0):
        super(EdgeLayer, self).__init__()
        self.edge_mlp = nn.Sequential(
            nn.BatchNorm1d(global_in + 2 * node_in + edge_in),
            nn.Linear(global_in + 2 * node_in + edge_in, edge_out),
    ...
    def forward(self, src, dest, edge_attr, u, batch=None):
...
class NodeLayer(nn.Module):
    def __init__(self, global_in, node_in, node_out, edge_in, leakiness=0.0):
        ...
        self.node_mlp_2 = nn.Sequential(
            nn.BatchNorm1d(global_in + node_in + node_out),
            nn.Linear(global_in + node_in + node_out, node_out),
            nn.LeakyReLU(negative_slope=leakiness),
            nn.BatchNorm1d(node_out),
            nn.Linear(node_out, node_out),
            nn.LeakyReLU(negative_slope=leakiness),
...
class GlobalLayer(torch.nn.Module):
    def __init__(self, global_in, node_in, global_out, leakiness=0.0):
        super(GlobalLayer, self).__init__()
        self.global_mlp = nn.Sequential(
            nn.BatchNorm1d(global_in + node_in),
            nn.Linear(global_in +node_in, global_out),
            nn.LeakyReLU(negative_slope=leakiness),
    ...
    def __init__(self):
        ...
        for i in range(self.num_mp):
            self.message_passing.append(
                MetaLayer(
                    edge_model = EdgeLayer(global_input, node_input, edge_input, edge_output, leakiness=leakiness),
                    node_model = NodeLayer(global_input, node_input, node_output, edge_output, leakiness=leakiness),
                    global_model = GlobalLayer(global_input, node_output, global_output, leakiness=leakiness)
                )
            )
        ...
        self.global_predictor = nn.Linear(global_output, 15)
...
global_loss = lossfn(output['global_pred'], data.global_label) # Edge classification loss
...
def train_gnn(loader, loaderTest, model, num_iterations=100, lr=0.001, optimizer='Adam', device='cpu'):
    ...
    losses, edge_losses, node_losses, global_losses= [], [], [], []
    ...
    while cur_epoch > 0:
        ...
        for data in loader:
            ...
            global_loss = lossfn(prediction['global_pred'], data.global_label) # Edge classification loss

            loss = node_loss + edge_loss + global_loss
    ...
    return np.array(losses), np.array(node_losses), np.array(edge_losses), np.array(global_losses), np.array(loss_x), np.array(node_accuracy), np.array(edge_accuracy), np.array(accuracy_epoch)
...
tstart = time.time()
# Train!
losses, node_losses, edge_losses, global_loss, loss_x, node_accuracy, edge_accuracy, accuracy_x = train_gnn(loader, loaderTest, model, num_iterations=epoch, device='cuda')
tend = time.time()
tLap = tend - tstart
h = int(tLap / 3600)
tLap = tLap % 3600
...
ax1.plot(loss_x, global_losses, 'y',label='global loss')
